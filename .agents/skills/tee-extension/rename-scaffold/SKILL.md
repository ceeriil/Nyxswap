---
name: rename-scaffold
description: "Rename the Hello World placeholders (HelloWorldInstructionSender, helloworld, module paths) in packages/tee-extension to the user's chosen extension name, across every language implementation present. Use when the user wants to rename or set up the Flare TEE (FCE) extension scaffold — e.g. \"rename this scaffold to Orderbook\", \"set up this extension as PriceFeed\"."
---

# Rename Scaffold

**Base directory:** `packages/tee-extension/` — every relative path and command below is relative to that directory (the one containing its `foundry.toml`); `cd` there first, or prefix paths accordingly.
