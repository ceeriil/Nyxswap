...
import (
	...
	"extension-scaffold/tools/pkg/contracts/nyxswap"
...
func DeployInstructionSender(s *support.Support) (common.Address, *nyxswap.NyxSwapInstructionSender, error) {
	...
	address, tx, contract, err := nyxswap.DeployNyxSwapInstructionSender(
...
func SetExtensionId(s *support.Support, instructionSenderAddress common.Address) error {
	sender, err := nyxswap.NewNyxSwapInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	if err != nil {
		...
		if reason == "" {
			parsed, _ := nyxswap.NyxSwapInstructionSenderMetaData.GetAbi()
			if parsed != nil {
