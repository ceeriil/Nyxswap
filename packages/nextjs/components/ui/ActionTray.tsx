...
// (e.g. a batched faucet claim across several tokens). Distinct from
// `notification` (~~/utils/scaffold-eth) - that's a single-message toast,
// this is a stack for a job with a known, fixed set of steps and per-step
// timing, which a single toast has no concept of.
// ─────────────────────────────────────────────────────────────
...
export function TrayProvider({ children }: { children: ReactNode }) {
  ...
  const start = useCallback(
    ({ title, steps }: { title: string; steps: string[] }): JobHandle => {
      ...
      let cursor = 0;
      const update = (mut: (j: Job) => Job) => setJobs(prev => prev.map(j => (j.id === id ? mut(j) : j)));
      ...
      const failStep = (reason?: string) => {
        ...
        update(j => ({
          ...j,
          steps: j.steps.map((s, i) => (i === at ? { ...s, status: "failed", detail: reason, endedAt: Date.now() } : s)),
        }));
      };
      ...
      const finish = ({ summary }: { summary?: Record<string, string> } = {}) => {
        update(j => ({
          ...
          steps: j.steps.map(s => (s.status === "failed" ? s : { ...s, status: "done", detail: undefined, endedAt: s.endedAt ?? Date.now() })),
...
const STEP_DOT: Record<StepStatus, string> = {
  pending: "border border-base-content/30",
  active: "bg-primary",
  done: "bg-success",
  failed: "bg-error",
  cancelled: "border border-base-content/30",
};
...
function TrayCard({ job, onDismiss }: { job: Job; onDismiss: () => void }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-base-300 bg-base-100 shadow-xl">
      <span className={cn("absolute inset-y-0 left-0 w-0.75", RULE_COLOR[state])} />
      <div className="flex items-center justify-between gap-2 border-b border-base-300 py-2 pl-4 pr-2">
        <span className="truncate text-xs font-medium uppercase tracking-wide">{job.title}</span>
        <div className="flex items-center gap-2">
          <span className="text-xs tabular-nums text-base-content/50">{secs((job.completedAt ?? now) - job.startedAt)}</span>
          <button onClick={onDismiss} aria-label="Dismiss" className="btn btn-ghost btn-circle btn-xs">
            &times;
          </button>
        </div>
      </div>
          ...
          <div key={i} className="flex items-center justify-between gap-2">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span
                className={cn(
                  "truncate text-xs",
                  s.status === "pending" || s.status === "cancelled" ? "text-base-content/50" : "text-base-content",
                )}
              >
                {s.label}
                {s.detail && <span className="text-base-content/50"> · {s.detail}</span>}
              </span>
            </div>
      ...
      {failed && <div className="border-t border-base-300 px-4 py-2 text-xs text-error">{job.error}</div>}

      {!failed && !running && job.summary && (
        <div className="flex flex-col gap-1 border-t border-base-300 px-4 py-2">
